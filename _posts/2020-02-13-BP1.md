...
------
To simply define what is a Riege regression. I want to refer the formula to get the optimal weights matrix.<br />
...
Now, let's take a look of how Riege Regression looks like:<br />
...
![Naive Bayes](https://github.com/superp0tat0/superp0tat0.github.io/raw/master/posts/post2/naive_bayes.png)
...
To find $$\beta$$ maximize $$LP$$, we could find the equivalent $$\beta$$ statisfy the following:
...
$$\beta = argmax_{\beta}LP\\
        ...
        = argmax_{\beta}(-\sum_{i=1}^{n} \frac{1}{2 \sigma^2}\left \| y_i - \beta x_i \right \|_{2}^{2} - \frac{\lambda}{2}\left \| \beta \right \|_{2}^{2} + C)\\
        = argmin_{\beta}(\sum_{i=1}^{n} \frac{1}{\sigma^2}\left \| y_i - \beta x_i \right \|_{2}^{2} + \lambda \left \| \beta \right \|_{2}^{2} + C)$$
...
Which is exactly the same as riege regression.

Now you should see the real function of what lambda does, it is the prior knowledge of the variance about $\beta$ follows a Gaussian distribion with 0 mean. With $$\lambda$$ to be higher, we are more convincing that $$\beta$$ will concentrate on 0 mean. It could smoothing our function for sure, but the disadvantage is that it will make our function to be a constant ($$\beta = 0$$). However, if you are convinced to be smooth to a particular weight matrix $$\alpha$$. For example, $$\beta \sim \mathcal{N}(\alpha,\lambda^{-1})$$. Then we could set our Riege regression to be:
$$argmin_{W}\left \| Y-\beta X \right \|_{2}^{2} + \lambda\left \| \beta - \alpha \right \|_{2}^{2}$$
