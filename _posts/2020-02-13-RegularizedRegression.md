...
<img scr='https://github.com/superp0tat0/superp0tat0.github.io/raw/master/files_posts/post2/naive_bayes.png'>
...
Now let's move on the connection between Naive Bayes to Ridge regression. To maximum our posterior given a set of observations. We only need to maximize $$likelihood*prior$$ Since the data probability will hold for all the parameters. To maximum the $likelihood*prior$. We could do some transformation to make our like easier. We define $$x_1,x_2, ... x_n$$ to be our observations and $$\beta$$ to be our hypothesis parameters, where we prefer $$\beta$$ to be a gaussian $$\beta \sim \mathcal{N}(0,\lambda^{-1})$$. We also define our target function to be: $$y_i = \beta x_i + \epsilon$$  where $$\epsilon \sim \mathcal{N (0,\sigma^2)$$.
